...
public class Server 
{
		...
		{ 
			...
			{ 
				...
				{ 
					...
					checkCorrectness(datamatrix,line);
				} 
				catch(IOException i) 
				{ 
	...
	void  checkCorrectness(ArrayList<ArrayList<Integer>> datamatrix ,String line) {
		...
		if(rows.size()==1&&cols.size()==1) {
			...
			int errorbit_index=rows.get(0)*9+cols.get(0);
			String corstr=line.substring(0,errorbit_index)+(1-Integer.parseInt(line.charAt(errorbit_index)+""))+line.substring(errorbit_index+1,line.length());
			System.out.println("Corrected String:"+convertToChars(corstr));
		}
	}
